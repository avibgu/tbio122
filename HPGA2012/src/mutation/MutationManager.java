...
//		rotation of -90 deg around y axis
    ...
    private boolean activateMutationOnProtein(Protein protein, Protein originalProtein, MutationLibraryEntry entry,int mutationStartMonomer, int mutationEndMonomer, Conformation originalConformation) {
        ...
        // The conformation(direction) of this momomer is not necessarily as written in mutation conformation,
        // therefore we will calculate its direction according to position of 3 vectors. 
    ...
    /*private boolean activateMutationOnProtein(Protein protein, Protein originalProtein, MutationLibraryEntry entry,int mutationStartMonomer, int mutationEndMonomer) {
    	numOfIterations++;    	
    	protein.reset();
    	protein.conformation.clear();
    	
    	Monomer mon;
    	for (int i = 0; i < mutationStartMonomer; i++){
    		mon = protein.get(i); 
    		mon.setR(new Vector3f(originalProtein.get(i).getR()));
    		protein.getGrid().update(mon);
    	}
    	    	    	    	
    	Vector3f mutationStartPos = originalProtein.get(mutationStartMonomer).getR();    	
    	for (int i = mutationStartMonomer, j = 0; i <= mutationEndMonomer; i++, j++){
    		mon = protein.get(i);
    		Vector3f vec = new Vector3f(entry.positionOffsets.get(j));
    		vec.add(mutationStartPos);
    		mon.setR(vec);    		
    		protein.getGrid().update(mon);
    	}
    	
    	for (int i = mutationEndMonomer + 1; i < protein.size(); i++){
    		mon = protein.get(i); 
    		mon.setR(new Vector3f(originalProtein.get(i).getR()));
    		protein.getGrid().update(mon);
    	}    	    	    		    	    	    	    
    	protein.updateFitness();
     ...
     * @param out - protein with low fitness. if mutation process is successful out will become the result of
     *              of the mutation, else out will be reset. 
    ...
    public void  mutate(Protein protein, Protein out, int max_tries) {
        ...
        while (nTries < max_tries)  { // upon success the method will return
            ...
            start = protein.get(mutationStartMonomer).getR(); // get position vector of start monomer.
            end = protein.get(mutationLastMonomer).getR();    // get position vector of end monomer.
            representativeVector.sub(end, start);
            
            // if start monomer and end monomer are on a straight line no mutation is possible.            
            ...
            if (list.size()> 0) {            	            
            	...
            	//  collect position of the protein before and after the mutation.
            	// save them not as absolut position but as offset from the star of mutation, this
            	...
            	for (int i = 0; i < protein.size(); i++){
        			if ((i < mutationStartMonomer || i > mutationLastMonomer)){
        				...
        				vec.sub(start); // Save position as an offset from start of mutation.
        				proteinPositions.add(vec);
        			}
            	}
            	// Save in candidates only the mutations that do no overlap with already occupied position of the protein.
                ...
                success = activateMutationOnProtein(out, protein, entry, mutationStartMonomer, mutationLastMonomer, originalConformation);
            	
            	/*while(!list.isEmpty() && !success){
            		selectedMutationIndex = selectMutationNumber(list);
                    entry =  list.remove(selectedMutationIndex);                    
                    success = activateMutationOnProtein(out, protein, entry, mutationStartMonomer, mutationLastMonomer, originalConformation);	
            	} */           	            	            
            }
            if (success) return;
